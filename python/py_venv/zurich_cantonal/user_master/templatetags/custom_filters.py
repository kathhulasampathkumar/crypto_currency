...
def getCurrencyFilter(userid):
    try:
        ...
        return '0'
@register.filter
def getInrToBtcFilter(userid):
    try:
        inr_to_btc_rate = 0.000000028  # Example conversion rate
        user = user_master.objects.get(delete1=0,id=userid)
        return "₿. "+str((int(user.balance) * inr_to_btc_rate))
    except user_master.DoesNotExist:
        return '₿. 0'

@register.filter
def getInrToUsdFilter(userid):
    try:
        inr_to_usd_rate = 78  # Example conversion rate
        user = user_master.objects.get(delete1=0,id=userid)
        return "$. "+str((int(user.balance) * inr_to_usd_rate))
    except user_master.DoesNotExist:
        return '$. 0'
@register.filter
def getInrFilter(userid):
    try:
        inr_to_usd_rate = 1  # Example conversion rate
        user = user_master.objects.get(delete1=0,id=userid)
        return "₹. "+str((int(user.balance) * inr_to_usd_rate))
    except user_master.DoesNotExist:
        return '₹. 0'
